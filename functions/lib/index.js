...
exports.backfillSummaries = exports.onEntryWrite = void 0;
...
async function applyIncrements(tx, docRef, increments) {
    ...
    if (!snap.exists) {
        ...
        if (increments.inCents)
            base.totalInCents = increments.inCents;
        if (increments.outCents)
            base.totalOutCents = increments.outCents;
        if (increments.count)
            base.count = increments.count;
    ...
    if (increments.count)
        updates.count = admin.firestore.FieldValue.increment(increments.count);
...
exports.backfillSummaries = functions.https.onRequest(async (req, res) => {
    ...
    const provided = req.query.secret || req.header('x-backfill-secret') || (req.body && req.body.secret);

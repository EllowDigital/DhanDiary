...
const ExportScreen = () => {
  ...
  const filteredForDate = (date: string | null) => {
    ...
    return (entries as LocalEntry[]).filter((e) =>
      dayjs(e.date || e.created_at).format('YYYY-MM-DD') === date
    );
  };
  ...
  const handleExport = async (fmt: 'pdf' | 'csv' | 'json') => {
    ...
    const summary = {
      ...
      currencySymbol: '₹',
      filterLabel: mode === 'All' ? 'All entries' : mode === 'Custom' ? `${customStart} → ${customEnd}` : mode,
    };
    ...
    try {
      ...
      if (fmt === 'pdf') {
        ...
        await FileSystem.writeAsStringAsync(fileUri, JSON.stringify({ metadata, summary, data: payload }, null, 2), {
          encoding: FileSystem.EncodingType.UTF8,
        });
        await Sharing.shareAsync(fileUri, { mimeType: 'application/json', dialogTitle: 'Share JSON Export' });
      }
            ...
            <View style={{ marginTop: 12, marginBottom: 8 }}>
              ...
              <View style={styles.chipsRow}>
                {['Day','Week','Month','Year','Custom','All'].map((m) => (
                  <Pressable key={m} style={[styles.chip, mode===m && styles.chipActive]} onPress={() => setMode(m as any)}>
                ...
                <View style={{ marginTop: 10 }}>
                  ...
                  <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingVertical: 8 }}>
                    {availableDates.slice(0, 30).map((d) => (
                      <Pressable key={d} style={[styles.chip, selectedDate===d && styles.chipActive]} onPress={() => setSelectedDate(d)}>
                        <Text style={[styles.chipText, selectedDate===d && styles.chipTextActive]}>{dayjs(d).format('DD MMM')}</Text>
                      </Pressable>
                    ))}
                  </ScrollView>
                </View>
              ...
              {mode === 'Custom' && (
                <View style={{ marginTop: 10 }}>
                  ...
                  <View style={{ flexDirection: 'row', gap: 8 }}>
                    <TextInput placeholder="Start" value={customStart} onChangeText={setCustomStart} style={[styles.chip, { flex: 1 }]} />
                    <TextInput placeholder="End" value={customEnd} onChangeText={setCustomEnd} style={[styles.chip, { flex: 1 }]} />
                  </View>
                </View>
              )}
              ...
              <View style={{ marginTop: 12 }}>
                ...
                <View style={styles.reportActions}>
                  <Pressable style={[styles.chip, format==='pdf' && styles.chipActive]} onPress={() => setFormat('pdf')}><Text style={[styles.chipText, format==='pdf' && styles.chipTextActive]}>PDF</Text></Pressable>
                  <Pressable style={[styles.chip, format==='csv' && styles.chipActive]} onPress={() => setFormat('csv')}><Text style={[styles.chipText, format==='csv' && styles.chipTextActive]}>CSV</Text></Pressable>
                  <Pressable style={[styles.chip, format==='json' && styles.chipActive]} onPress={() => setFormat('json')}><Text style={[styles.chipText, format==='json' && styles.chipTextActive]}>JSON</Text></Pressable>
                </View>
              </View>
              ...
              <View style={{ marginTop: 12 }}>
                ...
                <View style={{ flexDirection: 'row', gap: 10, marginTop: 8 }}>
                  <Pressable style={[styles.chip, includeNotes && styles.chipActive]} onPress={() => setIncludeNotes((v) => !v)}>
                    <Text style={[styles.chipText, includeNotes && styles.chipTextActive]}>{includeNotes ? 'Include notes' : 'Exclude notes'}</Text>
                  </Pressable>
                  <Pressable style={[styles.chip, groupBy==='none' && styles.chipActive]} onPress={() => setGroupBy('none')}><Text style={[styles.chipText, groupBy==='none' && styles.chipTextActive]}>No Group</Text></Pressable>
                  <Pressable style={[styles.chip, groupBy==='category' && styles.chipActive]} onPress={() => setGroupBy('category')}><Text style={[styles.chipText, groupBy==='category' && styles.chipTextActive]}>Group by Category</Text></Pressable>
                </View>
              </View>

              <View style={styles.actionsRow}>
                <Pressable style={[styles.actionBtn, styles.primaryBtn]} onPress={() => handleExport(format)} disabled={!!exporting}>

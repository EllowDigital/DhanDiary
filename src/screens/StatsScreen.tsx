...
import { Text } from '@rneui/themed'; // Ensure this package is installed
...
// Ensure asyncAggregator is default imported correctly
...
// Trillion-Safe Formatter (1.5Cr, 10M, 50k)
...
const StatsScreen = () => {
  ...
  const containerWidth = isTablet ? 600 : width - 32;
  const donutSize = Math.min(containerWidth * 0.6, 220);
  const innerSize = Math.round(donutSize * 0.65);
  const holeOffset = (donutSize - innerSize) / 2;

  // --- 1. DATE HELPERS (From your old code) ---
  const { availableMonths, availableYears } = useMemo(() => {
    ...
    entries.forEach((e) => {
      ...
      if (!monthMap.has(key)) {
        monthMap.set(key, { key, label: d.format('MMM YYYY'), date: d.startOf('month') });
        yearSet.add(d.year());
      }
    });

    return {
      availableMonths: Array.from(monthMap.values()).sort(
        (a, b) => b.date.valueOf() - a.date.valueOf()
      ),
      availableYears: Array.from(yearSet).sort((a: any, b: any) => b - a),
    };
  }, [entries]);
  ...
  // --- 3. ROBUST ASYNC CALCULATION ---
  const runAnalysis = useCallback(async () => {
    // Kill any running background task if filter changes quickly
    if (abortControllerRef.current) abortControllerRef.current.abort();
    const controller = new AbortController();
    abortControllerRef.current = controller;

    setComputing(true);
    // Important: Wait for navigation animation to finish before starting CPU work
    await new Promise((r) => InteractionManager.runAfterInteractions(() => r(null)));

    try {
      let result;
      // Trillion-Scale Protection: If data is massive, stream it instead of loading all at once
      ...
      if (result && !controller.signal.aborted) {
        ...
        const net = Number(result.net) / 100;
        const savingsRate = totalIn > 0 ? ((totalIn - totalOut) / totalIn) * 100 : 0;

        // Calculate daily average (safe division)
        ...
        setStats({
          ...
          // Convert Pie Data to safe UI format
      ...
      if (e instanceof Error && e.message !== 'Aborted') console.warn('Analysis Error', e);
    } finally {
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={['top', 'left', 'right']}>
      ...
      <View style={[styles.headerContainer, { width: containerWidth, alignSelf: 'center' }]}>
        <ScreenHeader
          title="Analytics"
      ...
      >
        ...
        >
                ...
                {filter === 'This Month' && (
                  ...
                  >
                    {availableMonths.map((m) => (
                      <Pressable
                        key={m.key}
                        style={[styles.timeChip, m.key === activeMonthKey && styles.timeChipActive]}
                        onPress={() => setActiveMonthKey(m.key)}
                      >
                        <Text
                          style={[
                            styles.timeChipText,
                            m.key === activeMonthKey && styles.timeChipTextActive,
                          ]}
                        >
                          {m.label}
                        </Text>
                ...
                {filter === 'This Year' && (
                  ...
                  >
                    {availableYears.map((y) => (
                      <Pressable
                        key={y}
                        style={[styles.timeChip, y === activeYear && styles.timeChipActive]}
                        onPress={() => setActiveYear(y)}
                      >
                        <Text
                          style={[
                            styles.timeChipText,
                            y === activeYear && styles.timeChipTextActive,
                          ]}
                        >
                          {y}
                        </Text>
          ...
          {/* 4. PERFORMANCE (Avg/Day & Txns) */}
          <View style={styles.gridContainer}>
            <View style={[styles.gridCard, { flex: 1 }]}>
              <View style={[styles.iconBox, { backgroundColor: '#FFF7ED' }]}>
                <MaterialIcon name="speed" size={18} color="#EA580C" />
              </View>
              <Text style={styles.gridLabel}>AVG / DAY</Text>
              <Text style={styles.gridValue}>
                {currencySymbol}
                {formatCompact(stats?.avgPerDay || 0)}
              </Text>
            </View>

            <View style={[styles.gridCard, { flex: 1 }]}>
              <View style={[styles.iconBox, { backgroundColor: '#F3F4F6' }]}>
                <MaterialIcon name="receipt" size={18} color="#4B5563" />
              </View>
              <Text style={styles.gridLabel}>TXNS</Text>
              <Text style={styles.gridValue}>{stats?.count || 0}</Text>
            </View>
          </View>
            ...
            >
              {stats?.dailyTrend?.length > 0 ? (
                <DailyTrendChart
                  ...
                  width={Math.max(containerWidth - 40, stats.dailyTrend.length * 45)}
                  currency={currencySymbol}
                />
              ) : (
                <View style={[styles.emptyChart, { width: containerWidth - 60 }]}>
...
const styles = StyleSheet.create({
  ...
  timeSlider: { paddingVertical: 8, paddingHorizontal: 4 },
  timeChip: {
    borderWidth: 1,
    borderColor: '#E2E8F0',
    borderRadius: 18,

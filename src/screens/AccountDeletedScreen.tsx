...
const AccountDeletedScreen = () => {
    ...
    const handleCreateAccount = () => {
        // Reset root to Auth -> Register
        const handleCreateAccount = async () => {
            // Reset root to Auth -> Register to prevent going back
            try {
                const { resetRoot } = await import('../utils/rootNavigation');
                resetRoot({ index: 0, routes: [{ name: 'Auth', state: { routes: [{ name: 'Register' }] } }] });
                return;
            } catch (e) {
                // fallback
            }

            // fallback local navigation reset
            navigation.reset({
                index: 1,

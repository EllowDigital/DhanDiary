...
const CashOutList = () => {
  ...
  const headerSubtitle = summary.count
    ? `${summary.count} expense entr${summary.count === 1 ? 'y' : 'ies'}`
    : 'Log every outgoing payment';
  ...
  const quickStats = useMemo(
    () => [
      ...
      { label: 'Avg spend', value: `₹${summary.avg.toFixed(0)}` },
      { label: 'Top category', value: summary.topCategory },
      { label: 'Last expense', value: lastActivityLabel },
    ],
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <ScreenHeader title="Cash Outflow" subtitle={headerSubtitle} />
